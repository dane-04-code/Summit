/**
 * The agent mark: the harness logo lookup, the accent's persistence
 * round-trip, and graceful degradation when a stored accent isn't one we know.
 *
...
describe('accent persistence', () => {
  it('round-trips the accent through the store', async () => {
    ...
    await repo.upsertAgent({ ...agent, accentColor: 'teal' });
  ...
  it('ships an additive migration for every post-release agent column', () => {
    ...
    expect(AGENT_COLUMN_MIGRATIONS.map((c) => c.name)).toEqual(
      expect.arrayContaining(['accent_color']),
    );
  });
});
...
describe('accent value resolution', () => {
  it('accepts every shipped accent name', () => {
    for (const name of agentAccentNames) expect(resolveAccent(name)).toBe(name);
  });

  it('rejects unknown, empty, and absent values', () => {
    for (const bad of ['ultraviolet', '', null, undefined, '__proto__']) {
  ...
  it('exposes ten distinct colors', () => {
...
describe('AgentAvatar', () => {
  const combinations: [string, string | null, string | null][] = [
    ['known harness + accent', 'hermes', 'teal'],
    ['known harness, no accent', 'openclaw', null],
    ['unrecognized framework + accent', 'openai', 'teal'],
    ['unrecognized framework, no accent', null, null],
    ['unrecognized accent value', 'hermes', 'ultraviolet'],
    // The prototype-chain trap: this must not resolve to a "valid" key.
    ['prototype-chain accent', 'hermes', '__proto__'],
  ];

  it.each(combinations)('renders with %s', async (_label, framework, accent) => {
    const view = await render(<AgentAvatar framework={framework} accent={accent} size={28} />);
    expect(view.toJSON()).toBeTruthy();
  });

  it.each(['hermes', 'openclaw'] as const)('draws the %s harness logo', async (framework) => {
    const view = await render(<AgentAvatar framework={framework} accent="teal" size={28} />);
    expect(view.toJSON()).toBeTruthy();
  });
});
...
describe('sidebar switcher rows', () => {
  const cases: [string, AgentOption[]][] = [
    ...
    [
      ...
      [
        { id: 'a1', name: 'Workshop', framework: 'hermes', frameworkLabel: 'Hermes', accentColor: 'teal' },
        { id: 'a2', name: 'Server box', framework: 'openclaw', frameworkLabel: 'OpenClaw' },
        { id: 'a3', name: 'Laptop', framework: 'openai', frameworkLabel: 'Ollama', accentColor: 'rose' },
        { id: 'a4', name: 'Spare', framework: 'hermes', frameworkLabel: 'Hermes' },
      ],
    ],
    [
      'agents carrying unrecognized values',
      [
        { id: 'a1', name: 'Workshop', framework: 'hermes', frameworkLabel: 'Hermes', accentColor: '#ff0000' },
        { id: 'a2', name: 'Server box', framework: 'openclaw', frameworkLabel: 'OpenClaw', accentColor: null },
      ],
    ],
  ];
...
describe('accent inside the drawer', () => {
  ...
  const UNMARKED: AgentOption[] = [{ id: 'a1', name: 'Workshop', framework: 'hermes', frameworkLabel: 'Hermes' }];
