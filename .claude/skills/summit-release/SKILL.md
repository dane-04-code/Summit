...
4. **Store metadata.** Owned by `summit-app-store` — hand off the listing fields, keywords, screenshots, privacy labels, and review notes rather than filling them here. Two things this skill must still track: metadata-only changes don't need a build (so a listing fix never blocks a release, and vice versa), and app review needs a working demo path for a pairing-gated app or it gets rejected for incomplete functionality — surface that prerequisite early.
